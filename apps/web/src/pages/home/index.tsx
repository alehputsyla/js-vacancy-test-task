...
const Home: NextPage = () => {
  ...
  const products = data && data.marketplaceCount > 0 && data.items.length > 0
    && (
  ...
  const emptyMarketplace = data?.marketplaceCount === 0
      && (
      <Container p={75}>
        <Text size="xl" c="gray">
  ...
  const emptySearch = data && data.marketplaceCount > 0 && data.items.length === 0
      && (
      <Container p={75}>
        <Text size="xl" c="gray">
  ...
  const filters = (
    <Skeleton radius="lg" visible={isListLoading}>
  ...
  const searchInput = (
    <Skeleton radius="md" visible={isListLoading}>
      <TextInput
  ...
  const contentHeader = (
    <Stack>
      <Group justify="space-between">
        <Skeleton visible={isListLoading} w="fit-content">
          <Text fw={700}>{`${data?.count || 0} results`}</Text>
        </Skeleton>
  ...
  const loadingState = isListLoading
      && (
      <SimpleGrid cols={{ base: 1, sm: 2, md: 3, lg: 2, xl: 3 }}>
        {[1, 2, 3, 4, 5, 6].map((key) => (
  ...
  const pagination = data && data.count > PER_PAGE && (
    <Center>
      <Pagination
        value={params.page}
